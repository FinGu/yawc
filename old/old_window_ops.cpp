...
void yawc_toplevel::set_fullscreen(bool enable){
    if (enable) {
        ...
        wlr_output *requested_output = nullptr;

        this->save_state();

        if(this->type == YAWC_TOPLEVEL_XDG){
            requested_output = this->xdg_toplevel->requested.fullscreen_output; 
        }
    ...
    if (this->type == YAWC_TOPLEVEL_XDG) {
        ...
        wlr_xwayland_surface_set_fullscreen(this->xwayland_surface, enable);
    }
...
void yawc_toplevel::default_set_maximized(bool enable){
    ...
    if(type == YAWC_TOPLEVEL_XWAYLAND){
        wlr_xwayland_surface_set_maximized(this->xwayland_surface, enable, enable);
    } else{
    ...
    if (type == YAWC_TOPLEVEL_XWAYLAND) {
        wlr_xwayland_surface_configure(this->xwayland_surface, 
             new_x, new_y, new_geo.width, new_geo.height);
    }
}

void yawc_toplevel::default_set_minimized(bool enable){
    if(type == YAWC_TOPLEVEL_XWAYLAND){
        wlr_xwayland_surface_set_minimized(this->xwayland_surface, enable);
    }
